...
export default function CreateProduct() {
  ...
  const handleCallApi = async () => {
    const url = `http://localhost:8000/api/ImportReceipt/checknopro?Address_store_id=${info_user.address_store_id}`;
    await axiosAuthentication.get(url).then(({ status, data }) => {
      if (status == 200) {
        setListProduct(data.model);
      }
    });
  };
  useEffect(() => {
