...
namespace Api.Controllers.OrdersController
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        [HttpPut("order_stop")]
        public async Task<IActionResult> StopOrder(string OrderId)
        {
            var check = await _order.StopOrder(OrderId);
            if (check.Status)
            {
                return Ok(check.Message);
            }
        ...
        [HttpGet("get-order-by-status-feed")]
        public async Task<IEnumerable<OrderSummary>> GetOrderByStatusFeed()
        {
            return await _order.GetOrderByStatusFeed();
        }

        [HttpPut("order-finish")]
        public async Task<IActionResult> UpdateOrderToFinish(string OrderId)
        {
            var check = await _order.UpdateOrderToFinish(OrderId);
            if (check)
            {
                return Ok(true);
            }
