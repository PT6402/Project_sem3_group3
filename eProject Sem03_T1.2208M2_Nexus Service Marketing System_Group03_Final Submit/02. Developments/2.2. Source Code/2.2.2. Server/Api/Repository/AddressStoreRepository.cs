	...
	public class AddressStoreRepository:IAddressStore
	{
		...
		public async Task<List<AddressDto>>GetAddress_Stores()
		{
			...
			foreach (var address in list) {
                var addressDto = _mapper.Map<AddressDto>(address.Addresses);
		...
		public async Task<string>Create(AddressDto model)
		{
			try
			{
                ...
                {
                    Address_store_id = newAddressStore.Id
                };
        ...
        public async Task<DtoResult<AddressStoreDto>> Delete(int id)
        {
            try
            {
                ...
                var storage = await _db.Storages.FirstOrDefaultAsync(s => s.Address_store_id.Equals(id));
                var import = await _db.ImportReceipt.Where(i => i.Storage_id == storage.Id).ToListAsync();
                if (addst != null)
                {
                    foreach (var item in import)
                    {
                        _db.ImportReceipt.Remove(item);
                        await _db.SaveChangesAsync();
                    }
